...
#endif
static void
verstrace_user(struct timeval* /*tvp*/,
               int TID,
               uint8_t lvl,
               const char* insert,
               const char* file,
               ...
               va_list ap)
{
  ...
  ers::LocalContext lc(reinterpret_cast<char*>(idx2namsPtr(TID)), file, line, function, DEBUG_FORCED);
  ...
  if (lvl < TLVL_DEBUG) { // NOTE: at least currently, TLVL_LOG is numerically 1 less than TLVL_DEBUG
    ...
    // PROTECTED - ers::Issue iss( ers::Issue( ers::Severity(ers::Log), tp, lc, outp, std::vector<std::string>(),
...
static void
erstrace_user(struct timeval* tvp,
              int TID,
              uint8_t lvl,
              const char* insert,
              const char* file,
...
inline void
operator<<(TraceStreamer& x, const ers::Issue& r)
{
  if (x.do_m) {
...
inline void
operator<<(TraceStreamer& x, const ers::InternalMessage& r)
{
  if (x.do_m) {
    x.line_ = r.context().line_number();
    x.msg_append(r.message().c_str());
    ...
    if (TRACE_INIT_CHECK(trace_name(TRACE_NAME, issue.context().file_name(), _trc_.tn, sizeof(_trc_.tn)))) {
      if (traceControl_rwp->mode.bits.M && (traceLvls_p[traceTID].M & TLVLMSK(lvl_))) {
        ...
        while ((issp = issp->cause())) {
          ...
          complete_message += ers::to_string(issp->severity()) + " [" + fbuf + " at " +
                              trace_path_components(issp->context().file_name(), 0) + ":" +
                              std::to_string(issp->context().line_number()) + "] " + issp->message();
        }
        trace(&lclTime,
              traceID,
              lvl_,
