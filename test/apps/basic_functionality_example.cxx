...
ERS_DECLARE_ISSUE_BASE(ers,           // namespace
                       ...
                       ((const char*)file_name),      // base class attributes // NOLINT
                       ((int)arg2)((const char*)arg3) // two attributes in this class // NOLINT
)

ERS_DECLARE_ISSUE(appframework,          // namespace
                  CommandNotRegistered2, // issue class name
                  "Command '" << command_name << "' does not have an entry in the CommandOrderMap!"
...
void
ex_thread(volatile const int* spinlock, int thread_idx)
{
  while (*spinlock)
    ; // The main program thread will clear this
...
int main(/*int argc, char *argv[]*/)
{
  ...
  // TLOG_DEBUG(6)<< ers::Message(ERS_HERE,"A LOG_DEBUG(6) using ers::Message - The Logging has just been setup. ERS bug
  // - 1st DEBUG is always DEBUG_0");
  ...
  TLOG() << "\ntshow follows:\n\n";
  system("TRACE_TIME_FMT=\"%Y-%b-%d %H:%M:%S,%%03d\" TRACE_SHOW=\"%H%x%N %T %e %l %L %m\" trace_cntl show | "
         "trace_delta -ct 1 -d 1");
