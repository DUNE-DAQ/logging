...
int
main(int argc, char* argv[] __attribute__((__unused__)))
{
  ...
  setenv("TRACE_LVLM",
         std::to_string(1ULL << (TLVL_DEBUG + dbglvl)).c_str(),
         0); // make sure the specific debug lvl used is enabled
  ...
  for (int ii = 0; ii < 10; ++ii)
    TLOG_DEBUG(dbglvl) << "message file does not exist"; // NOLINT

  TLOG() << "\ntshow follows:\n\n";                                                                 // NOLINT
  system("TRACE_SHOW=\"%H%x%N %T %P %i %C %e %L %R %m\" trace_cntl show | trace_delta -ct 1 -d 1"); // NOLINT
