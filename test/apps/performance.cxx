...
ERS_DECLARE_ISSUE(ERS_EMPTY, // namespace ERS_EMPTY ==> anonymous
                  ...
                  ((size_t)arg1)((int)arg2)((int)arg3)) // NOLINT
...
void
thread_func(volatile const int* spinlock, size_t thread_idx)
{
  ...
  if (g_do_issue) {
    ...
    for (auto uu = 0; uu < g_loops; ++uu)
      TLOG_DEBUG(lcllvl) << "tidx " << thread_idx << " fast LOG_DEBUG(" << lcllvl << ") #" << uu; // NOLINT
  }
...
{
  dunedaq::logging::Logging::setup(); // either do this or export DUNEDAQ_ERS_FATAL=erstrace,lstderr
  ...
  while (true) {
    ...
    switch (opt) {
      ...
      case 'h':
        ...
        break;
      case 'l':
        g_loops = static_cast<int>(strtoul(optarg, nullptr, 0));
        break;
      case 't':
        num_threads = static_cast<int>(strtoul(optarg, nullptr, 0));
        break;
      case 'i':
        g_do_issue = 1;
        break;
  ...
  if (g_do_issue) {
    ...
    for (int ii = 0; ii < g_loops; ++ii)
      TLOG_DEBUG(g_dbglvl) << "hello from DEBUG_" << g_dbglvl << " loop " << ii; // NOLINT
  }
